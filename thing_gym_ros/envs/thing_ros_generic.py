...
from thing_gym_ros.envs.gui.utils import ObjLockWrapper
...
class ThingRosEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self._poses_ref_frame = cfg['poses_ref_frame']
        self._rot_act_rep = cfg['rot_act_rep']
        ...
        self.image_width = cfg['img_width']
        self.image_height = cfg['img_height']
        self.image_zoom_crop = cfg['img_zoom_crop']
        ...
        assert cfg['max_policy_time'] < 1 / self._control_freq, "max_policy_time %.3f is not less than period " \
                                    "defined by control_freq %.3f" % (cfg['max_policy_time'], 1 / self._control_freq)
        self._max_policy_time = cfg['max_policy_time']
        self._fixed_time_after_action = 1 / self._control_freq - self._max_policy_time

        # resetting thresholds and parameters
        self._reset_base_tool_tf_arr = np.array(cfg['reset_base_tool_tf'])
        ...
        self._reset_odom_base_mat = self.tf_odom_base.as_mat()

        self._reset_joint_pos = np.array(cfg['reset_joint_pos'])
        ...
        if self._moving_base:
            self._base_theta_maximums = cfg['base_theta_maximums']  # each in rads
            self._cam_workspace_distance = cfg['cam_workspace_distance']
            self._base_reset_noise = cfg['base_reset_noise']
            ...
            self.base_cam_tf = rnt.tf_msg.TransformWithUpdate(self.tf_buffer, 'base_link', 'camera_link')
        ...
        # locks for things that can be modified by gui
        # self._reset_odom_base_mat = ObjLockWrapper(self._reset_odom_base_mat)
    ...
    def reset(self):
        ...
        while dist_arm_to_init > self._max_reset_trans:
            print("EE is %.3fm from initial pose. Must be within %.3fm." % (dist_arm_to_init, self._max_reset_trans))
        ...
        while rot_dist_arm_to_init > self._max_reset_rot:
            print("EE is %.3frad from init pose. Must be within %.3frad." % (rot_dist_arm_to_init, self._max_reset_rot))
            input("Move arm closer to initial pose and press Enter to continue...")
            self.tf_base_tool.update()
            rot_dist_arm_to_init = np.arccos(
        ...
        reset_thresh = .01
        if dist_arm_to_init > reset_thresh or rot_dist_arm_to_init > reset_thresh:
            ...
            if self._moving_base:
                dist_base_to_init = norm(self.tf_odom_base.as_mat()[:2, 3] - self._reset_odom_base_mat[:2, 3])

                # TODO continue here, base movement and arm movement should be simultaneous
        ...
        # generate observation for return
        obs = None
    ...
    def render(self, mode='human'):
        if mode == 'human':
            if self.gui_thread is None:
                ...
                self.gui_timer = rospy.Timer(rospy.Duration.from_sec(.1), self.__gui_timer_handler)
    ...
    def __gui_timer_handler(self, e):
        ...
        self.env_to_gui_q.put(dict(
            tf_odom_base_pos_quat=self.tf_odom_base.as_pos_quat(),
